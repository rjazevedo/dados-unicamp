...
def tratar_mun_nasc(df):
    for col in df.columns:
        if col in {"MUNICIPIO_NASC", "MU_NASC", "MUNIC_NASC", "CIDNASC", "CIDNAS"}:
            ...
            df["MUN_NASC"] = df["MUN_NASC"].map(
                lambda mun: unidecode(str(mun)).upper() if str(mun) != "-" else ""
            )
...
def tratar_uf_nasc(df):
    for col in df.columns:
        if col in {"UFNASC", "EST_NASC", "UFNAS"}:
            ...
            df["UF_NASC"] = df["UF_NASC"].map(
                lambda uf: unidecode(str(uf)).upper() if str(uf) != "-" else ""
            )
...
def tratar_mun_resid(df):
    for col in df.columns:
        if col in {"MUEND", "MUNIC_END", "MUNICIPIO", "CID", "CIDEND"}:
            ...
            df["MUN_RESID"] = df["MUN_RESID"].map(
                lambda mun: unidecode(str(mun)).upper()
            )
...
def tratar_opvest(df, date, path):
    ...
    if (date == 2001) or (date == 2002) or (date == 2003):
        emphasis = pd.read_excel(
            path, sheet_name="perfil", usecols=["insc_cand", "opcao1"], dtype=str
        )
        emphasis["insc_cand"] = pd.to_numeric(
            emphasis["insc_cand"], errors="coerce", downcast="integer"
        ).astype("Int64")
        ...
        df["OPCAO1"] = pd.to_numeric(
            df["OPCAO1"], errors="coerce", downcast="integer"
        ).astype("Int64")
...
def tratar_escola(df):
    ...
    for col in df.columns:
        if col in {"NOMEESC", "NOME_ESC", "NOME_ESCOLA", "ESCOLAEM", "ESCOLA", "ESC2"}:
            ...
            df["ESCOLA_EM"] = df["ESCOLA_EM"].map(
                lambda esc: unidecode(str(esc)).upper() if str(esc) != "-" else ""
            )
...
def tratar_mun_escola(df):
    ...
    for col in df.columns:
        if col in {
            ...
            df["MUN_ESC_EM"] = df["MUN_ESC_EM"].map(
                lambda mun: unidecode(str(mun)).upper() if str(mun) != "-" else ""
            )
...
def tratar_tipo_escola(df):
    ...
    for col in df.columns:
        if col in {"TIPOESC", "TIPO_ESC", "TIPO_ESCOL", "TIPO_ESCOLA"}:
            df["TIPO_ESCOLA_EM"] = pd.to_numeric(
                df[col], errors="coerce", downcast="integer"
            ).astype("Int64")
            df["TIPO_ESCOLA_EM"] = df["TIPO_ESCOLA_EM"].map(
                lambda tipo_esc: tipo_esc if tipo_esc in {1, 2} else pd.NA
            )
...
def tratar_ano_conclu(df, date):
    ...
    for col in df.columns:
        if col in {"ANO_CONCLU", "ANOCONC", "ANO_CONC", "ANO_CONCLUSAO"}:
            ...
            df["ANO_CONCLU_EM"] = df["ANO_CONCLU_EM"].map(
                lambda ano: "19" + str(ano) if len(str(ano)) == 2 else ano
            )
            df["ANO_CONCLU_EM"] = pd.to_numeric(
                df["ANO_CONCLU_EM"], errors="coerce", downcast="integer"
            ).astype("Int64")
...
def tratar_dados(df, date, path, ingresso=1):
    ...
    df["ANO_NASC"] = pd.to_numeric(
        df["ANO_NASC"], errors="coerce", downcast="integer"
    ).astype("Int64")
...
def extraction():
    ...
    for path, date in files.items():
        ...
        if date >= 2019:
            ...
            vi_dados = tratar_dados(
                vi_dados, date, path, ingresso=2
            )  # 2 - Vestibular Indigena
            vo_dados = tratar_dados(
                vo_dados, date, path, ingresso=3
            )  # 3 - Vagas Olimpicas
            ...
            if date != 2021:
                ...
                ve_dados = tratar_dados(
                    ve_dados, date, path, ingresso=4
                )  # 4 - ENEM-Unicamp
