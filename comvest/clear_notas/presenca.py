...
def get():
    ...
    for file in files:
        ...
        if 'F1' in file:
            ...
            df_pres.insert(0, column='ano_vest', value=int(file[:4]))
            df_pres['insc_vest'] = pd.to_numeric(df_pres['insc_vest']).astype('Int64')
            df_pres = df_pres.reindex(columns=['ano_vest','insc_vest','ppor','pbio','pqui','phis','pfis','pgeo','pmat','pest','papt','pcha','pcn','pinter'])
            f2.append(df_pres)
    ...
    com_presenca_f1 = df_notas[~df_notas['ano_vest'].isin([i for i in range(2005, 2011)])].loc[:, ['ano_vest','insc_vest','presente_f1']]
    f1.append(com_presenca_f1)

    com_presenca_f2 = df_notas[df_notas['ano_vest'].isin([i for i in range(1987, 2005)])].loc[:, ['ano_vest','insc_vest','ppor','pbio','pqui','phis','pfis','pgeo','pmat','pest','papt','pcha','pcn','pinter']]
    f2.append(com_presenca_f2)

    pres_f1 = pd.concat(f1)
    pres_f2 = pd.concat(f2)
    pres_vi = pd.concat([vi_2019, vi_2020, vi_2021]).loc[:, ['insc', 'sit']]

    pres_vi.columns = ['insc_vest','presente_vi']

    df_notas = df_notas.drop(columns='presente_f1').merge(pres_f1)
    df_notas = df_notas.drop(columns=['ppor','pbio','pqui','phis','pfis','pgeo','pmat','pest','papt','pcha','pcn','pinter']).merge(pres_f2, how='left')
