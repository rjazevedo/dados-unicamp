...
def filterby_joindate(df, year, how='exact'):
    if how == 'exact':
        ...
        return pd.concat([filterby_joindate(df, i) for i in range(year, int(np.max(df['ano_ingresso']))+1)])
    elif how == 'before':
        return pd.concat([filterby_joindate(df, i) for i in range(int(np.min(df['ano_ingresso'])), year+1)])

def filterby_exitdate(df, year, how='exact'):
    if how  == 'exact':
        return df[df['ano_saida'] == year]['id']
    elif how == 'after':
        return pd.concat([filterby_exitdate(df, i) for i in range(year, int(np.max(df['ano_saida']))+1)])
    elif how == 'before':
        return pd.concat([filterby_exitdate(df, i) for i in range(int(np.min(df['ano_saida'])), year+1)])
